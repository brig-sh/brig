...
brig keeps OAuth refresh tokens in a keychain and writes them into a guest by
design. A flaw in that path, or in the workspace and credential boundaries the
tool exists to hold, is worth telling us about privately before it is public.
This page is how.
...
<!-- MAINTAINER: if you want an email channel as well, add it here, e.g.
     security@brig.sh, and confirm the address exists and is monitored before
     merging. Left out on purpose rather than invented. -->
...
These are targets a small project can meet, not a contract. If you have not
heard back inside the response window, the reporting channel may not be enabled
...
Private vulnerability reporting is a repository setting, and it has to be turned
on for the link above to work. A maintainer enables it under
**Settings > Advanced Security > Private vulnerability reporting** (or
**Settings > Code security**, depending on the GitHub layout) for this
repository. Until then the **Report a vulnerability** button will not appear.
