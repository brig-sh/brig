...
| `BRIG_GATEWAY_DIR` | the directory of `BRIG_GATEWAY_SOCK`, else `~/.brig` | Where the gateway sockets live. A sandbox on `--network isolated` gets a gateway of its own here (`sandbox-<name>.sock`) on a `/30` of its own, rather than joining the shared one |
