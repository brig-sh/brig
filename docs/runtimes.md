...
| `hvi` | separate VM monitor for hull's `hvi` backend, which talks to Hypervisor.framework directly | a git submodule of hull (`hvi-vmm`), installed beside hull | unverified: the submodule URL is an SSH one and the repository did not answer from here |
...
| boot bundle | the kernel, `container-initrd` and the in-guest agent that let brig exec into an image built as an ordinary container. Published as an OCI artifact at `ghcr.io/nofireai/hull-assets`, one tag per guest platform | fetched by hull on macOS, by oras on Linux | unverified: the artifact is signed with keyless cosign, but its source repository did not answer from here, so its licence is not confirmed |

Two of those are unverified from this machine and are marked as such rather
than guessed. Everything else in the stack is Apache-2.0, including brig
itself.
