...
| `bash` | `brig sh` runs `bash -l`, and `brig sh <agent> <command...>` runs `bash -lc 'exec "$@"' bash <command...>`, for every profile regardless of its `binary:` field | `internal/wrap/run.go`, `shellArgv` |
