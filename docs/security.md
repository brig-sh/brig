...
`brig publish`, and on no other. A published port is the one inbound hole in
the boundary, it exists only because you asked for it, and it goes with
`brig unpublish`. See [Published ports](#published-ports) below.
...
a port. `--publish` on a run, and `brig publish` on a sandbox already up, are
the two ways to ask.
