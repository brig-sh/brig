...
| `network` | no | the sandbox's network posture: `shared` (the default: one network per host, and the sandboxes on it can reach each other) or `offline` (no route out at all). `BRIG_NETWORK`, `--network` and `--offline` win over it |
