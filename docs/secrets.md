...
probably not the page you want. brig resolves nothing itself -- it reads the
variables a profile names from its own environment -- so putting a value in
that environment is the whole integration, and it is a shorter road than
storing a second copy here:
...
## Stored is not forwarded, yet

Worth knowing before you build anything on top of it. A profile names
*environment variables*, and nothing yet connects those to the store, so
storing a value under the name `gh-token` does not make `GH_TOKEN` appear in
the guest. With the secret sitting in the store, it is still absent:
...
Until the profile side lands, compose the two yourself. `read` prints the
value, and the shell puts it in the environment of one command:

```console
$ GH_TOKEN=$(brig secret read gh-token) brig env claude
...
brig: forwarding to guest:
brig:   GH_TOKEN
brig:   CLAUDE_CODE_OAUTH_TOKEN(host)
...
`brig env` only reports what would be forwarded. The same line in front of
`brig run` is the real thing:

```bash
GH_TOKEN=$(brig secret read gh-token) brig run claude
```

Letting a profile reference a stored secret directly is
[#7](https://github.com/brig-sh/brig/issues/7).
...
Use it. The store does not reach the guest by itself yet, so the shell
composes the two:
...
`brig env` reports; `brig run` does it:
...
the next command carrying the composed line picks up the new value and the
sandbox does not need restarting.
