...
| "`BRIG_ALLOW_DENIED=false` does not forward the denied variable" (the fail-open switch) | security.md, Credentials (on an unmerged branch) | `pending:BRIG_ALLOW_DENIED false path, on a branch not yet merged` | pending |
