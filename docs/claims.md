...
| "`BRIG_ALLOW_DENIED=false` does not forward the denied variable" (the fail-open switch) | not in security.md yet, lands with #63 | `pending:BRIG_ALLOW_DENIED false path, on a branch not yet merged` | pending (fixed on #63, not merely untested: on this branch `BRIG_ALLOW_DENIED=false` still forwards the denied variable, so the claim is currently false; #63 is the fix) |
| "the digest that booted is the digest that was verified" | not in security.md yet, lands with the digest PR | `pending:digest pinning, pending the digest PR` | pending |
