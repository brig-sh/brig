...
This message is not brig's. It comes from the VM monitor when it loads against
a macOS that is missing a symbol it was built to call, which happens on an
older macOS than the runtime supports. The exact floor is not published yet.
What is known is that macOS 26 is the version brig is tested on. Update macOS,
and update hull (`brew upgrade`) so the two match.
