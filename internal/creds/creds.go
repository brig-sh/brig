...
// environment path: every variable a template names is read from brig's own
...
type Options struct {
	...
	AllowRefs bool
	// AllowDenied forwards a variable on the template's denylist. Deliberate,
	// because the denylist is what keeps a metered API key from silently
	// replacing a subscription token.
	AllowDenied bool
}

// Forward collects the variables named in `names` that are set and non-empty
// in the environment `lookup` reads.
//
// Unset and empty variables are skipped rather than forwarded empty, so they
// cannot shadow a value baked into the image.
func Forward(t agent.Template, names []string, lookup func(string) (string, bool), opt Options) Set {
	var s Set
		...
		if t.Denied(name) && !opt.AllowDenied {
			s.Warnings = append(s.Warnings, fmt.Sprintf(
				"not forwarding %s: it is on the %s denylist, because it outranks the "+
					...
					"billing without saying so. Set BRIG_ALLOW_DENIED=1 if that is what you want",
				name, t.Name))
			continue
		}
		if !opt.AllowRefs {
			if scheme, ok := unresolvedRef(value); ok {
				// direnv and friends readily leave a secret-manager reference
				// in the ambient environment unresolved. Forwarded verbatim it
				// yields "Invalid username or token" in the guest, which is
				// indistinguishable from a wrong username or a broken helper.
				// A real credential is a token and never takes this form.
				s.Warnings = append(s.Warnings, fmt.Sprintf(
...
// ReadHost resolves the template's host credential.
//
...
func ReadHost(hc *agent.HostCredential, command string) (*HostCredential, error) {
	if hc == nil {
		return nil, nil
	}
	var blob []byte
...
func walk(node any, field string) (any, bool) {
	...
	case map[string]any:
		...
		// the field appears once below this level. Templates name fields that
