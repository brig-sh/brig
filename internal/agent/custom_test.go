...
func reset(t *testing.T) {
	t.Helper()
	t.Cleanup(func() { custom = map[string]Template{} })
	custom = map[string]Template{}
}

// Export then import is the documented way to write a template, so it has to
...
func TestExportImportRoundTrip(t *testing.T) {
	...
	if !strings.Contains(string(blob), "# A brig agent template") ||
	...
	if err != nil {
		t.Fatalf("a template brig itself produced did not import: %v", err)
	}
	if back.Name != original.Name || back.Image != original.Image ||
		back.GuestHome != original.GuestHome || len(back.Forward) != len(original.Forward) ||
...
// A misspelled field would otherwise decode into nothing: a template that
...
`)
	...
	fromYAML, err := ParseTemplate(yamlBlob)
	if err != nil {
		t.Fatalf("YAML did not parse: %v", err)
	}
	fromJSON, err := ParseTemplate(jsonBlob)
	if err != nil {
		t.Fatalf("JSON did not parse: %v", err)
	}
	if fromYAML.Name != fromJSON.Name || fromYAML.Image != fromJSON.Image ||
		fromYAML.Mem != fromJSON.Mem || len(fromYAML.Forward) != len(fromJSON.Forward) {
		t.Errorf("the two spellings disagree:\n%+v\n%+v", fromYAML, fromJSON)
	}
}

// A template is a file someone wrote. Re-serialising it from the parsed
...
func TestImportPreservesWhatYouWrote(t *testing.T) {
	reset(t)
	dir := t.TempDir()
	blob := []byte(`# why this template exists: the vendored CLI needs a bigger guest
name: mine
image: docker.io/me/mine:latest
guestHome: /home/mine
binary: mine
...
`)
	...
	for _, comment := range []string{"why this template exists", "memory hog"} {
		if !strings.Contains(string(stored), comment) {
			t.Errorf("a comment was lost: %q", comment)
		}
	}

	// JSON keeps its own extension, and importing over a template of the same
...
func TestValidateRequiresTheEssentials(t *testing.T) {
	cases := map[string]Template{
		...
		"contradictory": {Name: "x", Image: "i", GuestHome: "/home/x", Binary: "x", Mem: 1, CPUs: 1,
			Forward: []string{"K"}, Deny: []string{"K"}},
	}
	...
	// A shell or gui template needs no binary.
	shell := Template{Name: "x", Image: "i", GuestHome: "/home/x", Shell: true, Mem: 1, CPUs: 1}
	if err := shell.Validate(); err != nil {
		t.Errorf("a shell template was rejected: %v", err)
	}
	// The missing-image error points at the documentation, because that is
	// the question someone writing their first template actually has.
	err := Template{Name: "x", GuestHome: "/home/x", Binary: "x", Mem: 1, CPUs: 1}.Validate()
...
func TestCustomTemplateOverridesABuiltIn(t *testing.T) {
	reset(t)
	dir := t.TempDir()
...
func TestLoadCustomReportsBadFilesAndKeepsGoing(t *testing.T) {
	reset(t)
	dir := t.TempDir()
	good := []byte("name: good\nimage: i\nguestHome: /home/g\nbinary: g\nmem: 1\ncpus: 1\n")
	...
	// Anything that is not a template is not a broken template.
	...
	if _, ok := Lookup("good"); !ok {
		t.Error("the good template was lost because another file was broken")
	}
}

// Most installs have no template directory at all.
func TestLoadCustomIgnoresAMissingDirectory(t *testing.T) {
	reset(t)
	if err := LoadCustom(filepath.Join(t.TempDir(), "nope")); err != nil {
