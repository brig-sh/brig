...
// defaultWorkspace is ~/brig/<template>, with one exception: a workspace the
// Homebrew wrapper created is adopted when it is already there, so an
// existing sandbox keeps its history, its logins and its checkouts across the
// cutover instead of silently starting empty next to it.
func defaultWorkspace(t agent.Template) string {
	home, err := os.UserHomeDir()
	if err != nil {
		home = "."
	}
	if legacy := legacyWorkspace(t); legacy != "" {
		if st, err := os.Stat(filepath.Join(home, legacy)); err == nil && st.IsDir() {
...
func legacyWorkspace(t agent.Template) string {
	switch t.Name {
	case "claude-code":
		return "sandboxed-claude"
	case "claude-desktop":
		return "sandboxed-claude-desktop"
	case "ubuntu":
		return "urunc-ubuntu"
	}
	return ""
