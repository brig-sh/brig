...
func (c *Config) openWorkspace() (*workspaceRoot, error) {
	// os.OpenRoot resolves the path it is handed, so a symlink AT the
	// workspace moves everything written below it somewhere else while every
	// path stays honestly "inside the root". That is the one escape the root
	// cannot see, so it is checked here instead, before anything is created.
	if info, err := os.Lstat(c.Workspace); err == nil && info.Mode()&os.ModeSymlink != 0 {
		target, _ := os.Readlink(c.Workspace)
