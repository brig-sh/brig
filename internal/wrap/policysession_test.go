...
// `attach -n` refuses a name it would have to rewrite, so a session row is
// only ever filed under `foo`; --name sanitises instead, so all three of
// these start the session `foo`. Each reaches `foo`'s policy, and its egress
// with it: a name that resolved without carrying the rules through would
// leave the same gap one layer down.
