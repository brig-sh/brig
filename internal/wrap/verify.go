...
// A runtime whose store is addressable by digest (nerdctl over containerd) can
// boot the exact bytes cosign checked, so it takes verifyDigest: resolve the
// tag to a digest, verify that digest, compare it against the copy on disk, and
// hand the digest on to boot. One whose store is not (hull) stays on verifyTag,
// which checks the tag as brig always has -- because claiming a digest was
// pinned when it was not is worse than the gap the claim would paper over. See
// runtime.Runtime.PinsDigest.
...
func (c *Config) verifyDigest() error {
	...
	case verify.NoTooling, verify.Unresolved:
		...
		// BootDigest empty is what makes that happen.
		if c.Verify == verify.Require {
			return fmt.Errorf("%s (BRIG_VERIFY=require)", res.Message())
		}
		c.warnf("%s", res.Message())
