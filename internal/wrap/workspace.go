...
func (c *Config) PrepareWorkspace() error {
	...
	if err := c.makeProjectionMounts(); err != nil {
		return err
	}
...
// makeProjectionMounts creates the guest-side mount points for the host
// directories we project read-only.
//
// Those paths sit inside the guest home, which is the workspace, so the
// directory the runtime mounts onto has to exist on the host first. Without
// it the guest either fails to mount or silently gets nothing, and the agent
// looks like it simply has no skills.
func (c *Config) makeProjectionMounts() error {
	for _, sh := range c.HostConfig {
		rel, err := filepath.Rel(c.Profile.GuestHome, sh.Guest)
		if err != nil || strings.HasPrefix(rel, "..") {
			// A projection outside the guest home needs no mount point from
			// us: it is not carved out of the workspace.
			continue
		}
		if err := os.MkdirAll(filepath.Join(c.Workspace, rel), 0o755); err != nil {
			return fmt.Errorf("cannot create the mount point for %s: %w", sh.Guest, err)
		}
	}
	return nil
}
