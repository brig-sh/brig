...
func (c *Config) seedHostConfig() error {
	for _, seed := range c.HostConfig {
		dst := filepath.Join(c.Workspace, seed.Rel)
		...
		for _, e := range entries {
			...
			if err := copyTree(filepath.Join(seed.Host, e.Name()), target); err != nil {
				return fmt.Errorf("cannot seed %s: %w", target, err)
			}
		}
	}
	return nil
...
// copyTree copies a file or directory, following neither symlinks nor the
// host's ownership. Symlinks are recreated as symlinks: a skill that points
// somewhere is the author's business, and resolving it here would copy in
