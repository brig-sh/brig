...
//
// What it says matters as much as what it does here. These two rows are the
// ones whose warn wording ends by naming what boots anyway, and the refusal
// used to be that sentence with the mode appended -- "Booting it unchecked
// (BRIG_VERIFY=require)" over a boot that never started.
...
func assertRefusalReadsAsOne(t *testing.T, what, msg string) {
	...
	// Its own remedy rather than namesAWayForward: these two rows are refused on
	// a host set up as its owner intended, so warn is the way past them, and the
	// shared helper deliberately does not accept turning the check down.
...
func TestGenericBootVerifiesTheBundle(t *testing.T) {
	...
	if err == nil {
		...
		// And it reads as a refusal about the kernel. The cause used to be the
		// image message nested whole, which named the image rather than the
		// bundle and carried "Booting it unchecked" inside a sentence that
		// refuses the boot.
	...
	// Warn is the default, and there it boots anyway -- which the line has to
	// say. The refusal above and this warning share a cause and part company on
	// exactly that clause.
