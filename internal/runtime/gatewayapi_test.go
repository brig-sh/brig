...
// must say the runtime is behind rather than quote a dial error.
func TestAGatewayWithNoAPISocketFailsOnlyARunThatPublishes(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "Upgrade the runtime") {
...
func TestStoppingASandboxReleasesItsHostPorts(t *testing.T) {
	...
	if got := Publications(sandbox); len(got) != 1 || got[0].HostPort != 18080 {
		t.Fatalf("stopping forgot what the sandbox publishes: %+v", got)
	}
	if err := reconcilePublications(g.sock, "198.18.0.5", Publications(sandbox)); err != nil {
...
func TestPublishWillNotTakeAnotherSandboxsPort(t *testing.T) {
	...
	if rec := Publications(sandbox); len(rec) != 0 {
