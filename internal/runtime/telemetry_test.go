...
// stubHull writes a stand-in for the hull binary that answers `telemetry
...
func stubHull(t *testing.T, state string) (*hull, func() string) {
	t.Helper()
...
func TestFreshInstallDoesNotCountTheBoot(t *testing.T) {
	h, logged := stubHull(t, "not configured (on by default; interactive runs will be asked)")
...
func TestDoNotTrackWins(t *testing.T) {
	...
	h, logged := stubHull(t, "disabled (DO_NOT_TRACK=1)")
...
func TestTerminalHandoverStaysAskable(t *testing.T) {
	h, _ := stubHull(t, "not configured (on by default; interactive runs will be asked)")
