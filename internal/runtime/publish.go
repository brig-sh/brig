...
// a user-mode gateway and asks it to forward; see forward.go. On Linux the
// container runtime publishes, which it can only do when the sandbox is
// created; see the note on nerdctl.Publish.
...
// to the loopback default. `brig unpublish 8080` names a port and no address;
// `brig unpublish 0.0.0.0:8080:80` names both.
...
// because the person typing `brig unpublish 8080` is naming the port they saw
// and cannot be asked to remember which interface it went to. One written with
// an address matches only that address.
...
// Same reports whether two publications occupy the same thing. Two of them
// cannot share a protocol and a host address, whatever guest port they carry
// to, because that is one listener on the host.
...
// `brig publish` takes its bare words.
//
...
// has to stop the run.
func ParsePublication(s string) (Publication, error) {
	...
	default:
		return Publication{}, fmt.Errorf("%s is not a port mapping; write it as PORT, "+
			"HOST:GUEST, or ADDR:HOST:GUEST", s)
	}
		...
		for _, have := range out {
			if have.Same(p) {
				return nil, fmt.Errorf("%s and %s both want %s on the host; "+
					"one host port carries one guest port", have, p, p.Local())
			}
		}
		out = append(out, p)
	}
