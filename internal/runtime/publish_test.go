...
func TestPublicationRecordSurvivesAndIsForgotten(t *testing.T) {
	...
	if got := Publications(name); len(got) != 0 {
	...
	if got := Publications(name); !reflect.DeepEqual(got, want) {
	...
	if got := Publications(name); len(got) != 2 || got[1].GuestPort != 90 {
...
func TestPublicationRecordIsPerSandbox(t *testing.T) {
	...
	if got := Publications("brig-a"); len(got) != 1 || got[0].HostPort != 8080 {
		t.Fatalf("brig-a: %+v", got)
	}
	if got := Publications("brig-b"); len(got) != 1 || got[0].HostPort != 9090 {
		t.Fatalf("brig-b: %+v", got)
	}
	ForgetPublications("brig-a")
	if got := Publications("brig-b"); len(got) != 1 {
