...
func supports(spec RunSpec, hv string) error {
	...
	if len(spec.Publish) > 0 && hv != "hvi" {
		return fmt.Errorf("this sandbox publishes %s, and brig opens a guest port at the "+
			...
			"%q); run it on hvi, or withdraw the port with `brig unpublish`. A publication "+
			"outlives the run that made it, so this applies whether or not --publish is on "+
			"this line", spec.Publish[0], hv)
	}
