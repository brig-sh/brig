...
//
// Two shapes reach here and they read the same to a user: a gateway that
// answers 404 because it predates the endpoint, and one with no API socket to
// dial because it was started without --api. Both mean the runtime is behind
// the brig driving it, and the fix for both is the same.
func unpublishable(err error) error {
	if !errors.Is(err, ErrNoForwardAPI) && !errors.Is(err, syscall.ENOENT) &&
		!errors.Is(err, syscall.ECONNREFUSED) {
		return err
	}
	return fmt.Errorf("this sandbox publishes a port, and the network gateway serving it "+
		"cannot publish one (%w). Upgrade the runtime, or withdraw the ports with "+
		"`brig unpublish`", err)
}
...
// posture a command resolved means `brig publish` finds the gateway the
// sandbox is actually attached to, which is the one that has to be told.
