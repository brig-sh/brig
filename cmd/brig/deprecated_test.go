...
// hostCredential: is removed. A file of the user's own that still carries it
// is refused at load, with the error naming the key and the command that
// replaces it, and every other command keeps working: an unusable profile is
// reported, not fatal, the way any other file that fails to parse is.
//
// Refused rather than parsed with the block dropped, because a profile that
// loaded without its credential would boot a sandbox with no login and no
// explanation -- the failure would surface inside the guest as a prompt to
// authenticate, the last place anyone connects back to a key in a file.
func TestHostCredentialIsRefusedAtLoad(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("BRIG_PROFILE_DIR", dir)
	blob := []byte("name: mine\nimage: i\nguestHome: /home/mine\nbinary: m\nmem: 1\ncpus: 1\n" +
	...
	var err error
	notice := captureStderr(t, func() {
		_, err = captureStdout(t, func() error { return run([]string{"agent", "ls"}) })
	})
	if err != nil {
		t.Fatalf("one unusable profile stopped every other command: %v", err)
	}
	for _, want := range []string{"mine.yaml", "hostCredential:", "removed", "brig secret import mine"} {
		if !strings.Contains(notice, want) {
			t.Errorf("the notice does not carry %q:\n%s", want, notice)
		}
	}
	if _, ok := profile.Lookup("mine"); ok {
		t.Error("a profile carrying hostCredential: loaded anyway")
	}
}
