...
// terminalStdin points os.Stdin at a character device, which is the only
// property wrap.IsTerminal tests -- so /dev/null stands in for a prompt, and a
// test can exercise the branch a user hits when they type the command with
...
// The question defaults to no, so a terminal that says nothing -- here a
// closed stdin, which reads as EOF -- leaves the secret alone.
...
// /dev/null stands in for a terminal on the other side, because IsTerminal
// tests os.ModeCharDevice and nothing else.
