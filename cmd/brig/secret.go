...
func writeSecret(args []string, verb string) error {
	...
	// creds.Forward already skips empty variables, so an empty secret could
	// never do anything but confuse whoever went looking for why.
	if len(value) == 0 {
...
// an unverified image.
//
// Both refusals name -y, because IsTerminal cannot tell the two apart as
// cleanly as it looks: a run with stdin on /dev/null is a character device and
// so counts as a terminal, asks a question nothing will answer, and reaches
// the EOF below. That is a cron job or a unit file, exactly the caller who
// needs to be told about the flag.
