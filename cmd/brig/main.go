...
usage:
  ...
  brig logs <ref> [--follow] [--tail N] [--raw]  stream the sandbox log (--gateway: the host gateway log)
...
func removeSandbox(cfg *wrap.Config, ref string) error {
	...
	if !present {
		// Prune anyway: there is no sandbox to remove, but the index entry names
		// one the user has asked to be rid of, and Forget is idempotent, so
		// clearing it here leaves no stale record of a name nothing holds.
		cfg.Forget()
