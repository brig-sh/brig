...
func reportPorts(cfg *wrap.Config, ref string, wantJSON bool) error {
	want := runtime.Publications(cfg.VMName)
	...
	for _, p := range want {
		rows = append(rows, portRow{Host: p.Local(), Guest: p.GuestPort, Protocol: p.Proto(), Live: live[p.Local()]})
	}
	if wantJSON {
