...
flags (before the agent's own arguments; -- ends brig's parsing):
  ...
  -d, --detach           with run: start the sandbox and exit
      --skills           seed your own ~/.claude skills and plugins into the
                         workspace, writable (or BRIG_SKILLS=1)
...
settings (BRIG_<AGENT>_<KEY> wins over BRIG_<KEY>; see the README for all):
  ...
  BRIG_SKILLS          1 to seed your ~/.claude skills and plugins in
...
func run(args []string) error {
	...
	if t.Unpublished && opts.load.Image == "" {
		return fmt.Errorf("we do not publish an image for %q, so there is nothing to boot.\n"+
			...
			"%s explains how to build one", t.Name, verb, t.Name, agent.BringYourOwnImageDoc)
	}
...
func parse(args []string) (o options, template string, tail []string, err error) {
	...
	for i < len(args) && err == nil {
		...
		case "--detach", "-d":
			...
			i++
		case "--skills":
			// Opt-in, and only ever opt-in: this copies the user's real
			// skills and plugins into the sandbox.
			o.load.Skills = true
			i++
		case "--":
			i++
...
func workspaceOf(vmName string, rt runtime.Runtime) string {
	...
	for _, name := range names {
		...
		// An unnamed sandbox is named exactly after its template, so there is
		// no session name to recover. TrimPrefix would leave the template
